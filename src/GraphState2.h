...
public:
	...
	void set_graph_from_file(string, string);
	void set_graph(GraphState2*);
	...
	void mlno_fit_graph(bool do_clear=true);
	void mlno_fit_graph_mlbt();
	bool mlno_fit_graph_mlno();
	bool mlno_reroot_at_outgroup();

	pair<bool, double> mlno_find_best_base_tree();
	bool mlno_is_labeled_treebased();
	...
	pair<bool, pair<int, int> > mlno_add_mig_to_base_tree_exhaustive();
	void mlno_get_vertices_below(Graph::vertex_descriptor v, set<Graph::vertex_descriptor> &vset);

	void mlno_compute_sigma_cor();
	void mlno_compute_sigma_cor_cov();
	void mlno_compute_sigma_cor_f2();
	void print_treeout(string stem);
